...
public class ProfilerTest : EditorWindow
{
    ...
    private bool ParsingCSV(ProfilerProperty profilerProperty, ref int f, ref List<List<string>> data, ref Dictionary<string, string> funcnameReplace, ref List<string> funcPathstack, ref List<KFunctionData> kFunctionStackData)
    {
        ...
        cacheFuncData2 = kFunctionStackData.FindAll(datas => datas.FunctionName.Equals("ParticleSystem.Update"));
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button("SetRoot一次"))
        {
            ...
            string profilerIndexPath = @"d:\0002.raw";
            Profiler.AddFramesFromFile(profilerIndexPath);

            ProfilerProperty profilerProperty = new ProfilerProperty();
            //profilerProperty.SetRoot(frame, UnityEditorInternal.Profiling.ProfilerColumn.SelfTime, UnityEditorInternal.Profiling.ProfilerViewType.Hierarchy);
            Debug.Log("SetRoot完毕，内存占用为：" + UnityEngine.Profiling.Profiler.GetTotalReservedMemoryLong() / 1024.0 / 1024.0 / 1024.0 + "GB");
            frame++;
        }

        if (GUILayout.Button("再SetRoot一次"))
        {
            Debug.Log("帧：" + frame);
            Debug.Log("ProfilerDriver.lastFrameIndex：" + ProfilerDriver.lastFrameIndex);
            if (frame > ProfilerDriver.lastFrameIndex)
            ...
            string profilerIndexPath = @"C:\Users\kingsoft\AppData\LocalLow\DefaultCompany\ProfilerDataCollectDemo\P2.raw";
            Profiler.AddFramesFromFile(profilerIndexPath);
        ...
        if (GUILayout.Button("原始快照解析"))
        {
            string path = "D:\\MemorySnapshot\\1650361968.snap";
        }

        if (GUILayout.Button("内存快照解析"))
        {
            int frame_id = 1;
            string SnapPath = "D:\\机甲内存快照\\第一局.snap";
            string UUID = "test";
            var p = UnityEditor.Profiling.Memory.Experimental.PackedMemorySnapshot.Load(SnapPath);
            var np = new PackedMemorySnapshot(p);
            Debug.Log(UUID);
            //SnapshotUtil.ConvertMemorySnapshotIntoJson(new PackedMemorySnapshot(p), frame_id, UUID);
        }

        if (GUILayout.Button("读一下Function数据"))
        {
            string test = string.Format("{0}:{1}", "10.11.146.82", 9000);
            Debug.Log(test);
        }
        ...
        if (GUILayout.Button("解析测试"))
        {
            ...
            Dictionary<int, CaseRender> allcasefunrow = new Dictionary<int, CaseRender>();
            List<Speedscope> mainList = new List<Speedscope>();
            List<Speedscope> renderList = new List<Speedscope>();
            List<CaseFlame> listFrame = new List<CaseFlame>();
            #endregion
            int frame_id = beginFrame;
            #region  原始数据
            ...
            for (int f = firstIndex; f < lastIndex; f++)
            {
                ...
                if (ParsingCSV(profilerProperty, ref f, ref data, ref funcnameReplace, ref funcPathstack, ref kFunctionStackData))
                {
                    ...
                    funcPathstack.Clear();
                    #region 处理主线程时序图数据
                    KProfilerHelper.GetProfilerFrameData(f, mainindex, ref profilerFrame);
                    KProfilerHelper.ExtractTimeLineData(profilerFrame, frame_id, mainindex, ref funcnameReplace, ref timedata, ref funcPathstack);
                    Speedscope mainthren = new Speedscope(0, UUID, threadMain, frame_id);
                    prt.GoToSpeedTree(timedata, threadMain, mainthren, allfun, ref funid);
                    mainList.Add(mainthren);
                    #endregion
                    funcnameReplace.Clear();
                    timedata.Clear();
                    funcPathstack.Clear();
                    ...
                    KProfilerHelper.ExtractTimeLineData(profilerFrame, frame_id, renderindex, ref funcnameReplace, ref timedata, ref funcPathstack);
                    mainthren = new Speedscope(0, UUID, threadRender, frame_id);
                    ren = prt.GoToSpeedTree(timedata, threadRender, mainthren, allfun, ref funid);
                    renderList.Add(mainthren);
                    ren = prt.GetRenFun(timedata, ref funhashMap);
                    ParsingTimelineRowjson(f, ref allcasefunrow, prt, ren, ref funhashMap); //渲染线程统计数据
                    ren.init();
                    #endregion
                    #region 释放空间
