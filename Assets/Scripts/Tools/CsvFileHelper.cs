...
public sealed class CsvFileHelper : IDisposable
{
    ...
    #region 编码方式可接受值
    //请考虑让应用程序使用 UTF-8 或 Unicode (UTF-16) 作为默认编码。大多数其他编码要么不完整并将许多字符转换为“?”，<br>        //要么在不同的平台上具有稍有不同的行为。非 Unicode 编码通常具有多义性，应用程序则不再试图确定合适的编码，也不再提供用户用来修复文本语言或编码的更正下拉菜单。
    /*
    ...
    */
    ...
    public static void SaveCsvFile(string yourFilePath, List<List<string>> yourDataSouse, bool isAppend, Encoding yourEncode)
    {
        ...
        {
            throw new Exception("在附加模式下，文件路径必须存在");
        }
        if (!isAppend && !File.Exists(yourFilePath))
        {
            if (yourFilePath.Contains('\\'))
            {
                ...
                if (!Directory.Exists(newPath))
                {
                    ...
                    //throw new Exception("文件路径或目录不存在");
                }
            }
